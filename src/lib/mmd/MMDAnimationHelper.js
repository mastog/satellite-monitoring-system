...
class MMDAnimationHelper {
  ...
  constructor(params = {}) {
    ...
    console.warn(
      "THREE.MMDAnimationHelper: The module has been deprecated and will be removed with r172. Please migrate to https://github.com/takahirox/three-mmd-loader instead."
    );
  }
  ...
  _setupMeshAnimation(mesh, animation) {
    ...
    if (animation !== undefined) {
      ...
      // Reads the looping clip's tracks from the current action so non-bone tracks can be reset between loops.
...
class GrantSolver {
  ...
  updateOne(grant) {
    ...
    if (grant.isLocal) {
      // Leaves local-position grants inactive because this helper currently applies only the supported grant paths.
      if (grant.affectPosition) {
      }

      // Leaves local-rotation grants inactive because this helper currently applies only the supported grant paths.
      if (grant.affectRotation) {
      }
    } else {
      // Leaves non-local position grants inactive because this helper currently applies only the supported grant paths.
      if (grant.affectPosition) {
      }
