...
class MMDLoader extends Loader {
  constructor(manager) {
    ...
    console.warn(
      "THREE.MMDLoader: The module has been deprecated and will be removed with r172. Please migrate to https://github.com/takahirox/three-mmd-loader instead."
    );
  }
...
// Defines shared loader helpers used by mesh, skeleton, morph, and material conversion routines.
...
// Initializes the runtime bone hierarchy from parsed MMD bone metadata before skinning is applied.
...
class GeometryBuilder {
  ...
  build(data) {
    ...
    // Builds IK metadata for PMD models with logic that currently parallels the PMX conversion path.
    ...
    if (data.metadata.format === "pmx") {
      ...
      // Builds a temporary parent-child tree so grant relationships can be traversed in dependency order.
        ...
        if (morph.type === 0) {
          ...
          for (let j = 0; j < morph.elementCount; j++) {
            ...
            if (morph2.type === 1) {
              ...
              // Leaves non-vertex group-morph entries untouched because this loader only applies supported morph categories.
            }
          }
          ...
          // bone
          // Leaves bone morphs unconverted because this loader's geometry pass only applies supported morph categories.
        } else if (morph.type === 3) {
          // uv
          // Leaves UV morphs unconverted because this loader's geometry pass only applies supported morph categories.
        } else if (morph.type === 4) {
          // additional uv1
          // Leaves additional-UV1 morphs unconverted because this loader's geometry pass only applies supported morph categories.
        } else if (morph.type === 5) {
          // additional uv2
          // Leaves additional-UV2 morphs unconverted because this loader's geometry pass only applies supported morph categories.
        } else if (morph.type === 6) {
          // additional uv3
          // Leaves additional-UV3 morphs unconverted because this loader's geometry pass only applies supported morph categories.
        } else if (morph.type === 7) {
          // additional uv4
          // Leaves additional-UV4 morphs unconverted because this loader's geometry pass only applies supported morph categories.
        } else if (morph.type === 8) {
          // material
          // Leaves material morphs unconverted because this loader's geometry pass only applies supported morph categories.
        }
      }
...
class MaterialBuilder {
  ...
  build(data, geometry /*, onProgress, onError */) {
    ...
    for (let i = 0; i < data.metadata.materialCount; i++) {
      ...
      params.specular = new Color().setRGB(
        ...material.specular,
        SRGBColorSpace
      );
